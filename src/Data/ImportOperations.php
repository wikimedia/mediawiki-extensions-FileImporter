...
class ImportOperations implements ImportOperation {
	...
	/** @var ImportOperation[] */
	private array $importOperations = [];
	/** @var int the state of this object, one of the class constants */
	private int $state = self::BUILDING;
	...
	public function prepare(): StatusValue {
		return $this->runOperations(
			...
			self::PREPARE_RUN,
			true,
			static fn ( ImportOperation $importOperation ) => $importOperation->prepare()
		);
	}
	...
	public function validate(): StatusValue {
		return $this->runOperations(
			...
			self::VALIDATE_RUN,
			false,
			static fn ( ImportOperation $importOperation ) => $importOperation->validate()
		);
	}
	...
	public function commit(): StatusValue {
		return $this->runOperations(
			...
			self::COMMIT_RUN,
			true,
			static fn ( ImportOperation $importOperation ) => $importOperation->commit()
		);
	}
